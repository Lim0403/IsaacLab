...
"""Evaluate hybrid raster search + PPO alignment policy.

This script evaluates the actual hybrid runtime structure:
1. Raster search
2. Switch to PPO alignment when Pt is detected
3. Evaluate success, final Pt, peak Pt, time-to-success, reward, alignment error, trajectory
...
parser = argparse.ArgumentParser(description="Evaluate hybrid raster+PPO alignment policy with RSL-RL.")
...
def read_env0_state_before_step(
    ...
    This avoids logging reset-contaminated states after the wrapper automatically resets done environments.
    """
...
def compute_episode_metrics(episode_id: int, rows: list[dict]) -> dict:
    if not rows:
        return {
            ...
            "align_duration_to_success": math.nan,
    ...
    align_duration_to_success = math.nan
    if not math.isnan(time_to_success) and not math.isnan(switch_time):
        align_duration_to_success = time_to_success - switch_time
    ...
    return {
        ...
        "align_duration_to_success": align_duration_to_success,
...
def compute_summary_metrics(episode_metrics: list[dict]) -> dict:
    ...
    align_times = [
        m["align_duration_to_success"]
        for m in success_rows
        if not math.isnan(m["align_duration_to_success"])
    ]
    switch_times = [m["switch_time"] for m in episode_metrics if not math.isnan(m["switch_time"])]
    ...
    return {
        ...
        "mean_align_duration_to_success_success_only": mean(align_times),
        "mean_switch_time": mean(switch_times),
...
def plot_pt_over_time(rows: list[dict], out_path: Path, success_threshold: float):
    ...
    switch_times = [r["time"] for r in rows if r["mode"] == "ALIGN"]
    if switch_times:
        plt.axvline(switch_times[0], linestyle=":", linewidth=1.5, label="Switch to ALIGN")
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    local_x, _ = get_local_robot_xy(env_unwrapped)
    ...
    print("=" * 80)
    print("[INFO] Hybrid evaluation started")
    ...
    while simulation_app.is_running() and episode_id < args_cli.num_episodes:
        ...
        with torch.inference_mode():
            ...
            local_x, local_y = get_local_robot_xy(env_unwrapped)
            ...
            if hybrid_mode == "SEARCH":
                ...
                if align_counter >= ALIGN_CONFIRM_STEPS:
                    hybrid_mode = "ALIGN"
                    print("=" * 80)
                    print(f"SWITCH TO ALIGN at episode={episode_id}, step={episode_step}, global_step={global_timestep}")
            ...
            # -----------------------------
            # Evaluation-side termination logic
            # -----------------------------
            if row["success_now"]:
            ...
            # -----------------------------
            # Step env only if evaluation episode is not already done
            # -----------------------------
            if not eval_done:
                ...
                # If the underlying env is done, also stop this evaluation episode.
            ...
            # -----------------------------
            # Finalize episode if evaluation says done
            # -----------------------------
            if row["eval_done"]:
                ...
                print(
                    ...
                    f"switch={metrics['switch_time']} "
                    f"align_t={metrics['align_duration_to_success']} "
                ...
                # Manual reset for next evaluation episode.
        ...
        if global_timestep % 50 == 0:
            receiver_x = env_unwrapped.receiver_x[0].item()
            receiver_y = env_unwrapped.receiver_y[0].item()
            print("-" * 80)
            print(f"global_step={global_timestep}, episode={episode_id}, episode_step={episode_step}, mode={hybrid_mode}")
            print(f"robot=({local_x:.4f}, {local_y:.4f})")
            print(f"receiver=({receiver_x:.4f}, {receiver_y:.4f})")
            print(f"pt={pt:.4f}, align_counter={align_counter}, eval_success_counter={eval_success_counter}")
